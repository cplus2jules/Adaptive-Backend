...
class ONNXOptimizer:
    ...
    def _export_and_load_models(self, pytorch_engine):
        ...
        try:
            ...
            try:
                ...
                logger.info("[OK] Llama ONNX session loaded (5x speedup expected)")
            ...
            if decoder_onnx_path.exists():
                try:
                    ...
                    logger.info("[OK] Decoder ONNX session loaded (4x speedup expected)")
                except Exception as e:
                    logger.error(f"Failed to load decoder ONNX session: {e}")
            
            logger.info("=" * 70)
            logger.info("ONNX Optimization Status:")
            logger.info(f"  Llama text2semantic: {'[OK] ENABLED' if self.llama_session else '[ERROR] DISABLED'}")
            logger.info(f"  VQ-GAN decoder: {'[OK] ENABLED' if self.decoder_session else '[ERROR] DISABLED'}")
    ...
    def _export_to_onnx(self, pytorch_model, model_name: str, dummy_input: Dict[str, torch.Tensor]):
        ...
        try:
            ...
            logger.info(f"[OK] ONNX export successful: {onnx_path}")
            return onnx_path
        except Exception as e:
    ...
    def load_llama_session(self, llama_model):
        ...
        try:
            ...
            logger.info("[OK] Llama ONNX session loaded")
            return True
        except Exception as e:
    ...
    def load_decoder_session(self, decoder_model):
        ...
        try:
            ...
            logger.info("[OK] Decoder ONNX session loaded")
            return True
        except Exception as e:
    ...
    def _export_llama_to_onnx(self, llama_model, output_path: Path) -> bool:
        ...
        try:
            ...
            logger.info(f"[OK] Llama model exported to ONNX: {output_path}")
            return True
    ...
    def _export_decoder_to_onnx(self, decoder_model, output_path: Path) -> bool:
        ...
        try:
            ...
            logger.info(f"[OK] Decoder model exported to ONNX: {output_path}")
            return True
    ...
    def synthesize(self, text: str, reference_audio: Optional[str] = None, **kwargs) -> Tuple[np.ndarray, int, Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"[OK] ONNX synthesis complete: {total_time*1000:.0f}ms, RTF: {rtf:.2f}")
