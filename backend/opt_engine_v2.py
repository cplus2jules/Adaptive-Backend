...
def _setup_fish_speech_path():
    ...
    if fish_speech_dir:
        ...
        if fish_speech_path.exists():
            ...
            print(f"[WARN] FISH_SPEECH_DIR set but path not found: {fish_speech_dir}")
...
class ThermalManager:
    ...
    def __init__(self, platform_name: str, cpu_tier: str = 'unknown'):
        ...
        if self.is_m1_air:
            logger.warning(
                "[WARN]  M1 MacBook Air detected. Performance will degrade after ~10 minutes "
    ...
    def _init_windows_monitoring(self):
        ...
        if self._try_libre_hardware_monitor():
            ...
            logger.info("[OK] LibreHardwareMonitor detected - thermal monitoring enabled")
            return
        
        # Try Core Temp
        if self._try_core_temp():
            self.monitoring_available = True
            logger.info("[OK] Core Temp detected - thermal monitoring enabled")
            return
        
        logger.warning("[ERROR] No thermal monitoring available - continuing without thermal protection")
    ...
    def get_throttle_warning(self) -> Optional[str]:
        ...
        if self.is_m1_air:
            ...
            if remaining <= 0:
                return "[WARN]  M1 Air: Thermal throttling active - performance reduced by 40-60%"
            elif remaining <= 120:  # 2 minutes warning
                return f"[WARN]  M1 Air: Thermal throttling expected in {int(remaining/60)} minutes"
...
class PerformanceMonitor:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info(f"[OK] NVML initialized successfully - GPU monitoring enabled (current: {test_util.gpu}%)")
        except ImportError:
            self.nvml_available = False
            logger.warning("[WARN] pynvml not installed - GPU utilization will show 0%. Install with: pip install nvidia-ml-py3")
        except Exception as e:
            self.nvml_available = False
            logger.warning(f"[WARN] NVML initialization failed - GPU utilization will show 0%: {type(e).__name__}: {e}")
    ...
    def __init__(self, 
        ...
        if not self.model_path.is_absolute():
            ...
            if not self.model_path.exists():
                ...
                if alt_path.exists():
                    ...
                    logger.info(f"[CHECK] Resolved model path to: {self.model_path}")
        ...
        if platform.system() == 'Darwin':  # macOS
            try:
                ...
                logger.info("[OK] macOS: Using 'spawn' method for multiprocessing")
                
                # Disable DataLoader workers if using MPS
                if self.device == "mps":
                    os.environ["PYTORCH_MPS_NO_FORK"] = "1"
                    logger.info("[OK] MPS: Disabled fork() to prevent deadlock")
            except Exception as e:
                logger.warning(f"[WARN] Could not configure macOS multiprocessing: {e}")
        ...
        if self.device == 'cpu':
            ...
            logger.warning("[WARN] torch.compile disabled on CPU (unreliable without compiler)")
        ...
        # ============================================
        logger.info("[DETECT] Force disabling gradient checkpointing...")
            ...
            try:
                ...
                if accessed_obj is not None:
                    ...
                    if hasattr(accessed_obj, 'config') or hasattr(accessed_obj, 'gradient_checkpointing_disable'):
                        model = accessed_obj
                        logger.info(f"[OK] Model found via {path_name}")
                        break
                    # Check if it has a model attribute
                    elif hasattr(accessed_obj, 'model'):
                        model = accessed_obj.model
                        logger.info(f"[OK] Model found via {path_name}.model")
                        break
            except Exception as e:
        ...
        if model is not None:
            ...
            if hasattr(model, 'config') and hasattr(model.config, 'use_gradient_checkpointing'):
                ...
                logger.info("[OK] Gradient checkpointing DISABLED via model.config")
                disabled_count += 1
            ...
            if hasattr(model, 'gradient_checkpointing_disable'):
                try:
                    ...
                    logger.info("[OK] Gradient checkpointing DISABLED via method")
                    disabled_count += 1
                except Exception as e:
                    logger.debug(f"Could not call gradient_checkpointing_disable(): {e}")
            
            if disabled_count > 0:
                logger.info(f"[TARGET] Successfully disabled gradient checkpointing ({disabled_count} method(s))")
            else:
                logger.warning("[WARN] Could not disable gradient checkpointing - may cause 30-40% slowdown")
        else:
            logger.warning("[INFO] Could not directly access model object (running in thread) - gradient checkpointing may still be enabled")
            logger.info("[WARN] If synthesis is slow, check that use_gradient_checkpointing=False in checkpoint config")
        
        # Apply torch.compile only on GPU (if enabled)
        # NOT on CPU - too unreliable due to compiler dependency
        if enable_compile:
            logger.info("[HOT] Compiling models with torch.compile...")
            try:
                self.decoder_model = torch.compile(self.decoder_model, mode='reduce-overhead')
                logger.info("[OK] Decoder model compiled")
                ...
                if hasattr(self.llama_queue, 'model'):
                    ...
                    logger.info("[OK] Llama model compiled")
            except Exception as e:
        ...
        # self._warmup()
        logger.info("[WARN] Warmup skipped to reduce startup time (saves 38+ seconds)")
        
        logger.info("[OK] OptimizedFishSpeechV2 initialized successfully!")
        ...
        try:
            ...
            if self.cpu_tier == 'm1_air':
                ...
                if throttle_state['throttled']:
                    logger.warning(f"[WARN]  {throttle_state['message']}")
