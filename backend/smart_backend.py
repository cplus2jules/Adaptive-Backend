...
class SmartHardwareDetector:
    ...
    def __init__(self):
        ...
        logger.info("[DETECT] Hardware detection complete")
    ...
    def _detect_wsl(self) -> bool:
        ...
        try:
            ...
            with open('/proc/version', 'r') as f:
                ...
                if is_wsl:
                    logger.info("[OK] Running in WSL2 - Triton support available!")
                return is_wsl
        except:
            return False
    ...
    def _detect_gpu(self) -> Dict[str, Any]:
        ...
        if torch.cuda.is_available():
            try:
                ...
                logger.info(f"[OK] NVIDIA GPU detected: {gpu_info['gpu_name']}")
            except Exception as e:
                logger.warning(f"CUDA detected but error: {e}")
        
        # Apple Silicon MPS - with disable option
        elif hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
            # Check if MPS is explicitly disabled
            disable_mps = os.getenv('DISABLE_MPS', '0') == '1'
            
            if disable_mps:
                logger.info("   Apple Silicon GPU detected but will not be used")
                # Keep gpu_info as CPU (default values)
            else:
                gpu_info.update({
                    'has_gpu': True,
                    'device_type': 'mps',
                ...
                logger.info("[OK] Apple Silicon GPU (MPS) detected")
    ...
    def _log_detection_results(self):
        ...
        logger.info(f"")
        logger.info(f"Thermal Monitoring: {'[OK] Available' if p.thermal_capable else '[ERROR] Not Available'}")
        logger.info(f"AVX-512 VNNI: {'[OK] Supported' if p.avx512_vnni else '[ERROR] Not Supported'}")
...
class ConfigurationSelector:
    ...
    def select_optimal_config(self) -> OptimalConfig:
        ...
        if self.profile.has_gpu and self.profile.gpu_memory_gb >= 6.0:
            logger.info(f"[OK] GPU configuration selected (GPU: {self.profile.gpu_name}, {self.profile.gpu_memory_gb:.2f}GB VRAM)")
            return self._gpu_config()
        
        # Log why GPU wasn't selected
        if self.profile.has_gpu:
            logger.warning(f"[WARN] GPU detected but insufficient VRAM: {self.profile.gpu_memory_gb:.2f}GB < 6.0GB required")
            logger.warning(f"[WARN] 4GB GPUs cause memory overflow (5.97GB usage) - using CPU mode instead")
            logger.info(f"[TIP] CPU mode with ONNX will provide better performance than overloaded GPU")
        else:
            logger.info("[INFO] No GPU detected, using CPU configuration")
    ...
    def _gpu_config(self) -> OptimalConfig:
        ...
        if self.profile.gpu_memory_gb <= 4.5:
            logger.warning(f"[WARN] 4GB GPU detected ({self.profile.gpu_name}) - applying extreme memory optimization")
            return OptimalConfig(
                ...
                notes='[WARN] Extreme memory optimization for 4GB VRAM - chunk_length=200, no quantization, fp16 only',
        ...
        # 1. WSL2 (Linux with Microsoft kernel) - [OK] Has Triton (best performance)
        # 2. Native Linux (NVIDIA GPU) - [OK] Has Triton (best performance)
        # 3. macOS (MPS) - [ERROR] UNSTABLE - causes hanging/freezing
        # 4. Windows - [ERROR] Triton not available, inductor unstable, disabled by default
        ...
        if force_compile:
            use_compile = True
            logger.info("[CONFIG] torch.compile FORCED via FORCE_TORCH_COMPILE=1")
        elif self.is_wsl and self.profile.has_gpu:
            use_compile = True
            logger.info("[INIT] WSL2 + NVIDIA GPU detected - enabling torch.compile with Triton (20-30% speedup!)")
        elif self.profile.system == 'Linux' and self.profile.device_type == 'cuda':
            # Native Linux with NVIDIA GPU
            use_compile = True
            logger.info("[INIT] Linux + NVIDIA GPU detected - enabling torch.compile with Triton")
        elif self.profile.system == 'Darwin':
            # macOS: torch.compile with MPS is UNSTABLE (causes hanging)
            use_compile = False
            logger.warning("[WARN] macOS detected: torch.compile disabled (MPS backend unstable, causes hanging)")
        elif self.profile.system == 'Windows':
            # Windows: Disabled (Triton not available, inductor unstable)
            use_compile = False
            logger.info("[WARN] Windows detected: torch.compile disabled (Triton not available, use WSL2 for 20-30% speedup)")
        else:
            # Unknown platform: disable for safety
            use_compile = False
            logger.warning(f"[WARN] Unknown platform ({self.profile.system}): torch.compile disabled for safety")
    ...
    def _m1_air_config(self) -> OptimalConfig:
        ...
        if device == 'mps':
            ...
            logger.info("[OK] M1 Air: Environment variables configured for MPS stability")
        ...
        return OptimalConfig(
            ...
            cache_limit=25,
            enable_thermal_management=True,
            expected_rtf=35.0, # Realistic RTF for M1 Air on CPU (30-40x range)
    ...
    def _high_end_cpu_config(self) -> OptimalConfig:
        ...
        return OptimalConfig(
            ...
            quantization='int8',
            useonnx=False, 
            usetorchcompile=False,  # Disabled: torch.compile on CPU requires C++ compiler, too unreliable
            chunk_length=512,
            max_batch_size=2,
    ...
    def _i5_baseline_config(self) -> OptimalConfig:
        ...
        return OptimalConfig(
            ...
            precision='fp32',
            quantization='int8',
            useonnx=False,  # [OK] Critical for i5
            usetorchcompile=False,  # Disabled: torch.compile on CPU requires C++ compiler, too unreliable
            chunk_length=512,
            max_batch_size=1,
    ...
    def _arm_sbc_config(self) -> OptimalConfig:
        ...
        return OptimalConfig(
            ...
            notes='[WARN] Proof of concept only - very slow',
...
class MemoryBudgetManager:
    ...
    def enforce_limits(self) -> bool:
        ...
        if available_gb < budget["safety_margin"]:
            logger.error(f"[WARN] CRITICAL: Only {available_gb:.1f}GB available (need {budget['safety_margin']}GB safety margin)")
            return False
...
class QuantizationStrategy:
    ...
    def get_quantization_config(cpu_tier: str, available_memory_gb: float) -> dict:
        ...
        if available_memory_gb < 6:
            ...
            logger.warning(f"[WARN] Aggressive quantization: INT4 (low memory: {available_memory_gb:.1f}GB)")
    ...
    def set_thread_affinity(self):
        ...
        try:
            ...
            if self.profile.cpu_tier == "intel_i5":
                ...
                logger.info(f"[OK] Pinned to P-cores: {p_cores}")
                ...
                logger.info(f"[OK] Pinned to cores: {cores}")
                ...
                logger.info(f"[OK] M1 Air: Using performance cores {perf_cores}")
...
class ResourceMonitor:
    ...
    def should_throttle(self, resources: Dict) -> bool:
        ...
        if resources['memory_percent'] > 90:
            logger.warning(f"[WARN] Memory usage critical: {resources['memory_percent']:.1f}%")
            return True
        
        if resources['cpu_percent'] > 95:
            logger.warning(f"[WARN] CPU usage critical: {resources['cpu_percent']:.1f}%")
            return True
    ...
    def suggest_adjustment(self, resources: Dict, current_config: OptimalConfig) -> Optional[OptimalConfig]:
        ...
        if self.should_throttle(resources):
            logger.info("[CONFIG] Auto-adjusting configuration to reduce resource usage")
...
class IntelligentDeviceSelector:
    ...
    def __init__(self, profile: HardwareProfile, user_preference: str = 'auto', device_locked: bool = False):
        ...
        logger.info(f"[SMART] Intelligent Device Selector initialized")
...
class SmartAdaptiveBackend:
    ...
    def __init__(self, model_path: str = "checkpoints/openaudio-s1-mini"):
        logger.info("[INIT] Initializing Smart Adaptive Backend")
        ...
        if self.user_device_preference != 'auto':
            logger.info(f"[USER] User device preference: {self.user_device_preference.upper()}")
            ...
            logger.info("[LOCK] Device locked to user preference (won't auto-switch)")
        else:
            logger.info("[BOT] Smart auto-selection enabled (will optimize based on system load)")
        ...
        if not self.monitor.memory_budget_manager.enforce_limits():
            logger.warning("[WARN] Memory constraints detected - using conservative settings")
        ...
        logger.info("[OK] Smart Adaptive Backend initialized successfully!")
        if not self.device_locked:
            logger.info("[TIP] Tip: Backend will auto-switch devices if system is overloaded")
            logger.info("[TIP] To lock device, set DEVICE=cpu or DEVICE=cuda in .env")
    ...
    def _log_selected_config(self):
        ...
        logger.info(f"ONNX Runtime: {'[OK] Enabled' if c.useonnx else '[ERROR] Disabled'}")
        logger.info(f"torch.compile: {'[OK] Enabled' if c.usetorchcompile else '[ERROR] Disabled'}")
    ...
    def _apply_user_device_preference(self, user_device: str):
        ...
        if user_device == 'cpu':
            logger.info("[OK] Forcing CPU mode (user preference)")
            ...
            if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
                ...
                logger.info("[LOCK] MPS backend disabled - using CPU only")
            ...
            if torch.cuda.is_available():
                ...
                logger.info("[BLOCK] CUDA disabled - GPU hidden, using CPU only")
            ...
            if torch.cuda.is_available():
                logger.info("[OK] Forcing CUDA mode (user preference)")
                self.profile.device_type = 'cuda'
                self.profile.has_gpu = True
            else:
                logger.warning("[WARN] CUDA requested but not available - falling back to CPU")
                self.profile.device_type = 'cpu'
                self.profile.has_gpu = False
        elif user_device == 'mps':
            if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
                logger.info("[OK] Forcing MPS mode (user preference)")
                self.profile.device_type = 'mps'
                self.profile.has_gpu = True
            else:
                logger.warning("[WARN] MPS requested but not available - falling back to CPU")
                self.profile.device_type = 'cpu'
                self.profile.has_gpu = False
        else:
            logger.warning(f"[WARN] Unknown device '{user_device}' - using auto-detection")
    ...
    def check_and_optimize_device(self) -> dict:
        ...
        if decision['should_switch']:
            logger.info(f"[REFRESH] Smart device switch: {self.engine.device} -> {decision['device']}")
    ...
    def _switch_device(self, new_device: str):
        ...
        try:
            ...
            logger.info(f"[REFRESH] Switching device: {old_device} -> {new_device}")
            ...
            logger.info(f"[OK] Device switched successfully to {new_device}")
        except Exception as e:
            logger.error(f"[ERROR] Failed to switch device: {e}")
            logger.warning(f"[WARN] Keeping device as {self.engine.device}")
    ...
    def _apply_configuration(self):
        ...
        logger.info(f"[CONFIG] Configuration applied")
    ...
    def _initialize_engine(self, model_path: str):
        ...
        if self.config.useonnx and self.config.device == 'cpu':
            try:
                ...
                logger.info("[LOAD] Loading Universal Optimizer with ONNX Runtime")
        ...
        logger.info("[LOAD] Loading Standard Optimized Engine V2")
    ...
    def tts(self, text: str, speaker_wav: Optional[str] = None, **kwargs):
        ...
        if adjusted_config:
            logger.warning("[WARN] System under pressure - auto-adjusting parameters")
        ...
        try:
            ...
            if "out of memory" in str(e).lower():
                logger.error("[BOOM] Out of memory! Retrying with conservative settings")
    ...
    def _log_performance(self, pre: Dict, post: Dict, result: Tuple):
        ...
        logger.info("[STATS] Performance Report:")
    ...
    def get_health(self) -> Dict[str, Any]:
        ...
        if resources['memory_percent'] > 80:
            insights.append("[WARN] Memory usage high - consider reducing batch size")
        if self.profile.cpu_tier == 'm1_air' and not self.profile.thermal_capable:
            insights.append("[WARN] M1 Air: Performance may degrade after 10-15 minutes")
        if not self.config.useonnx and self.config.device == 'cpu':
            insights.append("[TIP] ONNX Runtime could provide 4-5x speedup for CPU inference")
