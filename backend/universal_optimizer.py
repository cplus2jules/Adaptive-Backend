...
def print_platform_expectations(config: Dict[str, Any]):
    ...
    if 'note' in perf:
        logger.warning(f"  [WARN]  {perf['note']}")
...
class UniversalFishSpeechOptimizer:
    ...
    def __init__(self, model_path: str = "checkpoints/openaudio-s1-mini", device: str = None):
        ...
        if device is not None:
            logger.info(f"[LOCK] Overriding detected device '{self.config['optimizations'].get('device')}' with user preference: '{device}'")
    ...
    def _initialize_components(self, model_path: str):
        ...
        if opt.get('onnx_runtime', False):
            try:
                ...
                if check_onnx_availability():
                    ...
                    logger.info("[OK] ONNX Runtime optimizer ready (4-5x speedup expected)")
                else:
                    logger.warning("[WARN]  ONNX Runtime not installed - falling back to PyTorch")
                    logger.info("   Install with: pip install onnxruntime")
            except ImportError as e:
                logger.warning(f"[WARN]  ONNX Runtime not available: {e}")
                logger.info("   Install with: pip install onnxruntime")
        ...
        logger.info("[OK] Universal optimizer initialized successfully!")
    ...
    def _chunked_synthesis(self, text: str, reference_audio: str = None, **kwargs):
        ...
        logger.info(f"[OK] Chunked synthesis complete:")
